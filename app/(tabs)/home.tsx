...
const parseTripDate = (input: any): Date => {
  ...
  if (typeof input === "object" && "seconds" in input)
    return new Date(input.seconds * 1000); // Firestore Timestamp
  return new Date(0);
};
...
export default function HomeScreen() {
  ...
  // Fetch data (⬇️ removed SavingsService.list())
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setUser(userData);
      if (tripsData) dispatch(setTrips(tripsData));
      if (carsData) dispatch(setCars(carsData));
    } finally {
      setRefreshing(false);
  ...
  // ✅ Total Savings derived from trips (no separate store)
